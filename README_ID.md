...
<summary>Daftar nama Aplikasi yang akan di Debloated</summary>
...
- Masuk ke menu reboot
- Pilih fastboot reboot

Jalankan perintah ini (cmd-windows):
- `fastboot remove vendor`
...
1. Download:
   ...
   - [OrangeFox-MIUI (`.zip`)](https://github.com/basamaryan/android_device_xiaomi_sweet-TWRP/releases/download/R11.1_6/OrangeFox-R11.1_6-Unofficial-sweet-MIUI.zip) ➜ Simpan ke SDCard kamu.
3. Matikan ponsel kamu. Masuk ke mode fastboot dengan menekan dan menahan tombol **Power** + **Volume Down** secara bersamaan. Terus tahan hingga kamu masuk ke
mode fastboot, lalu lepaskan.

4. Setelah kamu masuk ke mode fastboot, sambungkan ponsel kamu ke PC menggunakan kabel USB.

5. Buka CMD dan ketik perintah:
`fastboot devices`
Lalu tekan Enter. Pastikan `device ID` kamu muncul, yang berarti perangkat kamu terdeteksi. Jika tidak terdeteksi, kamu perlu menginstal ulang `adb-fastboot drivers` dengan benar.

6. Untuk menginstal TWRP, gunakan perintah:
`fastboot flash recovery` (seret & letakkan file `twrp .img` di cmd) lalu tekan Enter.
- Contoh: `fastboot flash recovery twrp-3.7.1_12-0-sweet.img`

8. Untuk melakukan boot ulang ke TWRP, gunakan perintah:
